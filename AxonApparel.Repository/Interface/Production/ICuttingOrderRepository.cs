...
{
    public interface ICuttingOrderRepository
    {
        IQueryable<CuttingOrder> GetCuttingOrderInf();
        IQueryable<CuttingOrder> GetTrimsDetails(int CompanyId, int CompanyUnitId, string OrderType, string refno, int styleid, string ordo,int Buyerid);
        IList<CuttingOrder> GetMainData(int compid, int unitid, int buyerid, int masid, int empid, string refno, string orderno, string supptype, string ordtype, string cuttingordno, string jobordno, string FromDate, string ToDate,int supplierid);
        ...
        IList<CuttingOrder> GetAllIssueNo();
    }
}
