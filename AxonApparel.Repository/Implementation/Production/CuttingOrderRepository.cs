...
{
    public class CuttingOrderRepository : ICuttingOrderRepository
    {
        ProductionEntities entities = new ProductionEntities();

        public IQueryable<CuttingOrder> GetTrimsDetails(int CompanyId, int CompanyUnitId, string OrderType, string refno, int styleid, string ordo, int Buyerid)
        {
            IQueryable<CuttingOrder> query = (from T in entities.proc_Apparel_CuttingOrderAddlistingforBulk(CompanyId, CompanyUnitId, OrderType, refno, styleid, ordo, Buyerid)
        ...
        public IList<CuttingOrder> GetMainData(int compid, int unitid, int buyerid, int masid, int empid, string refno, string orderno, string supptype, string ordtype, string cuttingordno, string jobordno, string FromDate, string ToDate, int supplierid)
        {
            var query = (from a in entities.proc_Apparel_GetCuttingOrderDetailsForJobMaingrid(compid, unitid, buyerid, masid, empid, refno, orderno, supptype, ordtype, cuttingordno, jobordno, FromDate == null ? "" : FromDate.ToString(), ToDate == null ? "" : ToDate.ToString(), supplierid)
                         select new CuttingOrder
                         {
        ...
        public IList<CuttingOrder> GetAllIssueNo()
        {
            IQueryable<CuttingOrder> queryColor = (from T in entities.Proc_Apparel_GetProdAllIssuNo()
                                                   select new CuttingOrder
                                                   {
                                                     CuttingOrdId = T.CuttingOrdid,
                                                     CuttingOrdNo=T.CuttingOrderNo
