...
{
    public class CuttingOrderReceiptRepository : ICuttingOrderReceiptRepository
    {
        ...
        public IQueryable<CuttingReceipt> GetCuttingRecptDetails(int CompanyId, int CompanyUnitId, string OrdType, string refno, int styleid, string ordo, int buyerid, string jobordno, string inorext)
        {
            IQueryable<CuttingReceipt> query = (from T in entities.Proc_Apparel_GetCuttingReceipt(CompanyId, CompanyUnitId, OrdType, refno, styleid, ordo, buyerid, jobordno, inorext)
                                                select new CuttingReceipt
                                                {
        ...
        {
            var query = (from a in entities.Proc_Apparel_GetCuttingReceiptMainDetails(ID, InterExternal, OrderType, FromDate == null ? "" : FromDate.ToString(), ToDate == null ? "" : ToDate.ToString(),
                            companyid, jobordno, orderno, refno, employeeid, unitid)
                         select new CuttingReceipt
                         {
