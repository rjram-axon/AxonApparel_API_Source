...
{
    public class BitCuttingOrderReturnRepository : IBitCuttingOrderReturnRepository
    {
        ProductionEntities entities = new ProductionEntities();
        ExceptionLogging exceplogg = new ExceptionLogging();

        public IQueryable<CuttingReturn> GetCuttingReturnDetails(int CompanyId, int CompanyUnitId, string OrdType, string refno, int styleid, string OrderNo, int buyerid, string jobordno, string inorext,int Processid)
        {
            IQueryable<CuttingReturn> query = (from T in entities.Proc_Apparel_GetBitCuttingReturnDetails(CompanyId, CompanyUnitId, OrdType, refno, styleid, OrderNo, buyerid, jobordno, inorext, Processid)
        ...
        public IList<CuttingReturn> GetMainData(int ID, string OrderType, string InterExternal, string FromDate, string ToDate, string jobordno, string orderno, string RefNo, int Supplierid, int employeeid,int Processid)
        {
            var query = (from a in entities.Proc_Apparel_GetBitCuttingReturnMainDetails(ID, InterExternal, OrderType, FromDate == null ? "" : FromDate.ToString(), ToDate == null ? "" : ToDate.ToString(), jobordno, orderno, RefNo, Supplierid, employeeid, Processid)
                         select new CuttingReturn
                         {
