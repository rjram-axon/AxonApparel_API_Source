...
{
    public interface IBitCuttingOrderBusiness
    {
        Response<IQueryable<CuttingOrder>> GetCuttingOrderInfo();
        Response<IQueryable<CuttingOrder>> GetCuttingOrderDetails(int CompanyId, int CompanyUnitId, string OrderType, string refno, int styleid, string ordo, int Buyerid,int Processid);
        Response<IList<Domain.CuttingOrder>> GetMaindt(int compid, int unitid, int buyerid, int masid, int empid, string refno, string orderno, string supptype, string ordtype, string cuttingordno, string jobordno, string FromDate, string ToDate,int supplierid,int Processid);
