...
{
    public interface ICuttingOrderReceiptBusiness
    {
        Response<IQueryable<CuttingReceipt>> GetCuttingReceiptInfo();
        Response<IQueryable<CuttingReceipt>> GetCuttingOrderDetails(int CompanyId, int CompanyUnitId, string OrdType, string refno, int styleid, string ordo, int buyerid, string jobordno, string inorext);
