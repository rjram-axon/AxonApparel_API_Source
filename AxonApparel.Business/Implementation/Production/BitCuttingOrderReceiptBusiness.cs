...
{
    public class BitCuttingOrderReceiptBusiness : IBitCuttingOrderReceiptBusiness
    {
        IBitCuttingOrderReceiptRepository CuttingDet = new BitCuttingOrderReceiptRepository();
        ...
        public Response<IQueryable<CuttingReceipt>> GetCuttingOrderDetails(int CompanyId, int CompanyUnitId, string OrdType, string refno, int styleid, string ordo, int buyerid, string jobordno, string inorext,int Processid)
        {
            try
            {
                var CuttingOrderrecDt = CuttingDet.GetCuttingRecptDetails(CompanyId, CompanyUnitId, OrdType, refno, styleid, ordo, buyerid, jobordno, inorext, Processid);
            ...
            string orderno, string refno, int employeeid, int unitid,int Processid)
        {
            try
            {
                var getmaindt = CuttingDet.GetMainData(Id, OrderType, InterExternal, Fromdate, Todate, companyid, jobordno, orderno, refno, employeeid, unitid, Processid);
