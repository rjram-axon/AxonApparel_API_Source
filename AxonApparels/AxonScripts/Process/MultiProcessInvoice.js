...
$(document).ready(function () {
    ...
    LoadMProcess();
    LoadMUnit();
    var fill = localStorage.getItem('MultiProcessInvoiceMainFilter');
    ...
    //    for (var t = 0; t < POSaveItemList.length; t++) {
    //        if (POSaveItemList[t].Process_recpt_DetId == CSno) {
    //            pid.push(POSaveItemList[t].Process_recpt_DetId);
    //            bal.push(POSaveItemList[t].RecQty);
    //            qty.push(POSaveItemList[t].InvoiceQty);
    //        }
    //    }
    ...
    //    for (var u = 0; u < POSaveItemList.length; u++) {
    //        for (var r = 0; r < pid.length; r++) {
    //            if (POSaveItemList[u].Process_recpt_DetId == pid[r]) {
    //                POSaveItemList[u].InvoiceQty = qty[r];
    //            }
    //        }
    //    }
    ...
    //    var difrate = parseFloat(Rate - Grnrate);
    //    var damnt = parseFloat(difrate * Invqty);
    //    var qtydif = parseFloat(Invqty - Balqty);
    //    var qtyamnt = parseFloat(qtydif * Rate);
    //    var test = [];
    //    if (CreDebSaveList.length == 0) {
    //        var obj = {
    //            Proc_Recpt_Masid: Masid,
    ...
    $("#tblProInvAddgrid").dataTable().find("tbody").on('click', 'td', function () {
        debugger;
        $('input[id=groupbomAdd]').each(function () {
            var row = $(this).closest('tr');
            if ($(this).is(':checked')) {
                var val = $(this).val();
                for (var d = 0; d < AItemList.length; d++) {
                    if (AItemList[d].ProcRecId == val) {
                        AItemList[d].CheckType = "Y";
                    }
                }
       ...
       // MGRowID = 0;
        ...
        for (var d = 0; d < AItemList.length; d++) {
            ...
            if (AItemList[d].CheckType == "Y") {

                MGPrRowID = MGPrRowID + "," + AItemList[d].processid;
                MGRowID = MGRowID + "," + AItemList[d].ProcRecId;
            }
        }
    ...
    //    var difrate = parseFloat(Rate - Grnrate);
    //    var damnt = difrate * Invqty;
    //    var qtydif = parseFloat(Invqty - Balqty);
    //    var qtyamnt = qtydif * Rate;
    ...
    //    $.each(POSaveItemList, function () {
    //        if (this.Process_recpt_DetId == pid) {
    //            //if (balq >= value) {
    //            this.InvoiceQty = value;
    //            //}
    //            //else {
    ...
    //            var Clr = PESaveItemList[g].Color;
    //            var Size = PESaveItemList[g].Size;
    //            var Invqty = PESaveItemList[g].Invoice_Qty;
    //            var Rate = PESaveItemList[g].Rate;
    //            var Masid = PESaveItemList[g].Proc_Recpt_Masid;
    //            var Balance = PESaveItemList[g].BalQty;
    //            var Grnrate = PESaveItemList[g].GrnRate;
    ...
    //$('#tblProInvAddgrid').on('click', 'tr', function (e) {


    //    var table = $('#tblProInvAddgrid').DataTable();

    //    var row = $(this).closest('tr');
    //    var data = $('#tblProInvAddgrid').dataTable().fnGetData(row);
    //    //processid

    //    var ItmId = data.processid;

    //    MGPrRowID = MGPrRowID + "," + ItmId;
    ...
    //    var ProcRecId = table.row($(this).parents('tr')).data()["ProcRecId"];

    //    var val = $(this).is(":checked");
    //    if (val == true) {
    //        for (var f = 0; f < AItemList.length; f++) {
    //            if (AItemList[f].processid == Processid && AItemList[f].ProcRecId == ProcRecId) {
    //                //MGPrRowID = 0;
    //                //var ItmId = AItemList[f].processid;

    //                MGPrRowID = MGPrRowID + "," + AItemList[f].processid;
            ...
            { title: "GrnMasID", data: "ProcRecId", "visible": false },
              { title: "Processordid", data: "processordid", "visible": false },
               { title: "Processid", data: "processid", "visible": false },
                 ...
                 //        return '<input type="checkbox" id="txtGrnMasId" class="editor-active"  style="width: 50px;text-align: center;"  value=' + data + ' onclick="myprofunc(this.value);">';
...
//function myprofunc(Val) {
//    MGPrRowID = MGPrRowID + "," + Val;
//}
...
function LoadProInvEntryDetails() {
    ...
    GenerateNumber();
    LoadPrnInvDetails(MGRowID, CompId, SuppId, UnitId, MGPrRowID);
...
function LoadPrnInvDetails(MGRowID, CompId, SuppId, UnitId, ProcessId) {
    ...
    var UnitId = UnitId;
    $.ajax({
        url: "/ProcessInvoice/MultiGetInvPrnItemDetails",
        ...
        success: function (result) {
            ...
            LoadProInvItemSaveDetails(MGRowID, CompId, SuppId, UnitId, MGPrRowID, PType, OrdType);
...
function LoadProInvItemSaveDetails(MGRowID, CompId, SuppId, UnitId, ProcessId, PType, OrdType) {
    debugger;
    $.ajax({
        url: "/ProcessInvoice/MultiGetProInvSaveItemDetails",
        data: JSON.stringify({ PrnMasId: MGRowID, CompanyId: CompId, SupplierId: SuppId, ProcessId: ProcessId, UnitId: UnitId, InternalOrExternal: PType, OrderType: OrdType }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function ProInvSave() {
    ...
    $.each(PESaveItemList, function () {
        TotitemInvqty = TotitemInvqty + parseFloat(this.Invoice_Qty);
    });

    $.each(POSaveItemList, function () {
        TotordInvqty = TotordInvqty + parseFloat(this.InvoiceQty);
    });


    if (TotitemInvqty != TotordInvqty) {
...
function LoadMainGrid() {
    ...
    var menufilter = OrdType + ',' + comId + ',' + FDate + ',' + TDate + ',' + ProcessId + ',' + UnitId + ',' + ProcessorId + ',' + PrnMasId + ',' + ProcessInvId + ',' + OrdNo + ',' + RefNo + ',' + 'Y';
    localStorage.setItem('MultiProcessInvoiceMainFilter', menufilter);

    $.ajax({
        url: "/ProcessInvoice/GetMainLoad",
        data: JSON.stringify({ OrderType: OrdType, CompanyId: comId, FromDate: FDate, ToDate: TDate, ProcessId: ProcessId, UnitId: UnitId, SupplierId: ProcessorId, PrnMasId: PrnMasId, Process_Invid: ProcessInvId, OrdNo: OrdNo, RefNo: RefNo, MultiFlag: 'Y' }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function LoadMainGridFromBack() {
    ...
    $.ajax({
        url: "/ProcessInvoice/GetMainLoad",
        data: JSON.stringify({ OrderType: fillobj[0], CompanyId: fillobj[1], FromDate: fillobj[2], ToDate: fillobj[3], ProcessId: fillobj[4], UnitId: fillobj[5], SupplierId: fillobj[6], PrnMasId: fillobj[7], Process_Invid: fillobj[8], OrdNo: fillobj[9], RefNo: fillobj[10], MultiFlag: 'Y' }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (json) {
...
function ProInvDelete() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Value == true) {
                AddInvBillNo("N");
                AddUserEntryLog('Process', 'Multi Process Invoice', 'DELETE', $("#txtEntryno").val());
...
function SubReport() {
    ...
    //window.open("../ReportInline/Process/ProcessInvoice/ProcessInvoiceInlineReport.aspx?Masid=" + Repid + "&Debcred=" + p[0] + "&Rem=" + p[1] + "&Style=" + p[2] + "&Pendingdet=" + p[3] + "&Ewaybill=" + p[4] + "&Ewaydate=" + p[5] + "&Companyid=" + comp);
    var comp = $('#ddlMCompany').val();
    if (comp != null) {
        window.open("../ReportInline/Process/ProcessInvoice/ProcessInvoiceInlineReport.aspx?Masid=" + Repid + "&Debcred=" + p[0] + "&Rem=" + p[1] + "&Style=" + p[2] + "&Pendingdet=" + p[3] + "&Ewaybill=" + p[4] + "&Ewaydate=" + p[5] + "&Companyid=" + comp + "&Type=" + "M");
    }
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.Value == true) {
                        ...
                        AddUserEntryLog('Process', 'Multi Process Invoice', 'ADD', $("#txtEntryno").val());
                        //alert("Data Saved Sucessfully");
                        var msg = 'Data Saved Sucessfully...';
                        var flg = 4;
                        var mod = 1;
...
function MultiProInvUpdate() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Value == true) {
                AddInvBillNo("Y");
                AddUserEntryLog('Process', 'Multi Process Invoice', 'UPDATE', $("#txtEntryno").val());
                //alert("Data Updated Sucessfully");
                var msg = 'Data Updated Sucessfully...';
                var flg = 1;
                var mod = 0;
