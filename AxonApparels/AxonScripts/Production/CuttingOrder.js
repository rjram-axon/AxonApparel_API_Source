...
$(document).ready(function () {
    ...
    var fill = localStorage.getItem('CuttingOrderMainFilter');
   ...
   // LoadData(companyid, fromdate, todate);
    ...
    $(document).on('keyup', '.txtallqty', function () {
        ...
        //var allototalvalue = 0;
        //for (var e = 0; e < Ilist.length; e++) {
        //    var amountlst = Ilist[e].issqty;
        //    if (Ilist[e].ItemId == itmid && Ilist[e].ColorId == colorid && Ilist[e].SizeId == sizeid) {
        //        allototalvalue = allototalvalue + parseFloat(amountlst);
        //    }
        //}
        ...
        $('#tblinputgrd').on('click', 'tr', function (e) {
            ...
            if (Mode == 1) {
                ...
                if (tmplist != undefined && tmplist.length > 0) {
                    ...
                    $.ajax({
                        url: "/CuttingOrder/GetInputItemStockEditMode",
...
function LoadItemStock(Jobordno, Companyid, IssueStoreid, Styleid, Colorid, Itemid, Sizeid) {
    ...
    $.ajax({
        url: "/CuttingOrder/GetInputItemStock",
...
function getbyID(CuttingOrderId) {
    ...
    $.ajax({
        url: "/CuttingOrder/GetCuttingHeaderInfo/" + CuttingOrderId,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
...
function Add() {
    ...
    //if (CostBudCutAppid == 'Y') {
    //    $.each(Olist, function (e) {
    //        if (Olist[e].ordqty > 0) {
    //            if (Olist[e].rate > 0 && Olist[e].rate <= Olist[e].apprate) {
            ...
            success: function (result) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        var flg = 1;
                        var mod = 0;
                        var ur = "/CuttingOrder/CuttingOrderIndex";
                        AlartMessage(msg, flg, mod, ur);
                        //$('#ddlcutCompany').val(0);
                        // $('#ddlcutunit').val(0);
                        $('#myModal1').modal('hide');
...
function UpdateDetail() {
    ...
    //    $.each(IOlist, function (e) {
    //        if (IOlist[e].ordqty > 0) {
    //            if (IOlist[e].rate > 0 && IOlist[e].rate <= IOlist[e].apprate) {
...
function fnLoadInOutGridOnEditMode(CuttingOrderId, prodprgid) {
    ...
    $.ajax({
        type: "POST",
        url: "/CuttingOrder/GetInOutDetEdit",
...
function LoadCuttingOrder(companyid, companyunitid) {
    ...
    $.ajax({
        type: "POST",
        url: '/CuttingOrder/ListCuttingOrder/',
...
function LoadData() {
    ...
    var menufilter = CompId + ',' + Unit + ',' + buyer + ',' + mas + ',' + emp + ',' + RecNo + ',' + OrdNo + ',' + supptype + ',' + ordtype + ',' + DCNo + ',' + jordNo + ',' + FDate + ',' + TDate + ',' + suppid;
    localStorage.setItem('CuttingOrderMainFilter', menufilter);

    //$('#tblcuttingmaingrid').DataTable().destroy();
    $.ajax({
        type: "POST",
        url: '/CuttingOrder/GetMaindt/',
        //data: JSON.stringify({ CompanyId: companyid, CompanyUnitId: companyunitid }),
        data: JSON.stringify({ compid: CompId, unitid: Unit, buyerid: buyer, masid: mas, empid: emp, refno: RecNo, orderno: OrdNo, supptype: supptype, ordtype: ordtype, cuttingordno: DCNo, jobordno: jordNo, Fromdate: FDate, Todate: TDate, supplierid: suppid }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
...
function LoadDataFromBack() {
    ...
    $.ajax({
        ...
        url: '/CuttingOrder/GetMaindt/',
        //data: JSON.stringify({ CompanyId: companyid, CompanyUnitId: companyunitid }),
        data: JSON.stringify({ compid: fillobj[0], unitid: fillobj[1], buyerid: fillobj[2], masid: fillobj[3], empid: fillobj[4], refno: fillobj[5], orderno: fillobj[6], supptype: fillobj[7], ordtype: fillobj[8], cuttingordno: fillobj[9], jobordno: fillobj[10], Fromdate: fillobj[11], Todate: fillobj[12], supplierid: fillobj[13] }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
...
$(document).on('click', '.CuttOrdPrint', function () {
    ...
    window.open("../ReportInline/Production/CuttingOrderReportInline/CuttingOrdReportInline.aspx?CuttingOrdId=" + CuttingOrdId + "&Companyid=" + compid + "&type=" + 'C');
});
...
function LoadTotalFabwgt() {

    var totfab = 0;

    $.each(Ilist, function (i) { 
        totfab =totfab +parseFloat(Ilist[i].issqty);
  
    });

    $('#txttotFabwgt').val(totfab);
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (obj.length > 0) {
                    var data = obj;
                    $(ddlprocessor).empty();
                    $(ddlprocessor).append($('<option/>').val('0').text('--Select Supplier--'));
                    $.each(data, function () {
                        $(ddlprocessor).append($('<option></option>').val(this.SupplierId).text(this.SupplierName));
                    });
                }
            }
        });
