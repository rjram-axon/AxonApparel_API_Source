...
var PanelProcessReturnEditFlg = "disabled";
...
$(document).ready(function () {
   ...
   // LoadSupplierDDL("#ddlMProcessor");
    ...
    $('#tblcuttingreturnmaingrid').on('click', 'tr', function (e) {
        ...
        var CuttingReturnNo = data.CuttingReturnNo;
        LoadItemMovements(CuttingReturnNo);
    });
    $(document).on('click', '.btnmaingrddelete', function () {
        ...
        $.ajax({
            url: "/BitCuttingReturn/ReceiptHeaderInfo/",
    ...
    $(document).on('click', '.btnmaingrdedit', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/BitCuttingReturn/ListCuttingReturnOpDetEditDetails',
                ...
                $.ajax({
                    type: "POST",
                    url: '/BitCuttingReturn/ListCuttingReturnWastageDetailsEdit',
    ...
    $(document).on('click', '.btnordadd', function () {
        ...
        $.ajax({
            type: "POST",
            url: '/BitCuttingReturn/ListCuttingReturnHeaderDet',
            ...
            success: function (json) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/BitCuttingReturn/ListCuttingReturnDetDetails',
                ...
                $.ajax({
                    type: "POST",
                    url: '/BitCuttingReturn/ListCuttingReturnOpDetDetails',
                    data: JSON.stringify({ Issueid: CuttingIssueId }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (json) {
    ...
    $(document).on('keyup', '.txtretqty', function () {
        ...
        var otable = $('#tblinnergrid').DataTable();
        var odata = otable.rows().data();
        $('input[id=txtretqty]').each(function (ig) {
            if (odata[ig].CuttingIssueDetid == Csno && odata[ig].Itemid == itm && odata[ig].Colorid == clr && odata[ig].Sizeid == sze) {
                var row = $(this).closest('tr');
                row.find('#txtretqty').focus().val('').val(ReturnQty);
            }
        });
...
function Delete(ID) {
    ...
    if (ans) {
        LoadingSymb();
        $.ajax({
            url: "/BitCuttingReturn/Delete/" + ID,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (result) {
                ...
                //LoadData(companyid, OrderType, InterExter, fromdate, todate);
                CuttReturnId = 0;
                //window.location.href = "/BitCuttingReturn/BitCuttingReturnIndex";
                var msg = 'Record deleted successfully...';
                var flg = 2;
                var mod = 0;
                var ur = "/BitCuttingReturn/BitCuttingReturnIndex";
                AlartMessage(msg, flg, mod, ur);
            },
            error: function (errormessage) {
                alert(errormessage.responseText);
...
function LoadDDL(companyid, ordertype, interexter, fromdate, todate) {
    ...
    $.ajax({
        type: "POST",
        url: '/BitCuttingReturn/GetMaindt/',
        data: JSON.stringify({ CompanyId: companyid, OrderType: OrderType, InterExternal: InterExter, Fromdate: fromdate, Todate: todate, jobordno: jordNo, orderno: ordNo, Refno: RefNo, Supplierid: suppid, employeeid: empid,Processid:Process }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
...
$(document).on('click', '.CuttReturnPrint', function () {
    ...
    window.open("../ReportInline/Production/CuttingReturnReportInline/CuttingReturnReportInline.aspx?CuttingReturnid=" + CuttingReturnid + "&Companyid=" + compid +"&type=" + 'P' );
});
...
function LoadCuttingOrderReceipt() {
    ...
    $.ajax({
        type: "POST",
        url: '/BitCuttingReturn/ListCuttingOrderReturn',
        //data: JSON.stringify({}),
        ...
        success: function (json) {
            ...
            $('#tblinnercuttingreturn').DataTable({
                ...
                columns: [
                         ...
                         { title: "Rec Balance", data: "Balance" },
                         {
                             title: "ACTION", "mDataProp": null,
                             "sDefaultContent": '<button type="button" type="button" data-toggle="tooltip" data-placement="top" title="Add" style="width: 20px;padding: 0px;height: 20px;border-radius:10px;" class="btnordadd btn btn-round btn-success" > <i class="fa fa-plus"></i> </button>'
                         }
