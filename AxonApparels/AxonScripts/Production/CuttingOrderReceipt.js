...
var editsubstore = 0;
var CompanyId = 0;
var CuttingRecptEditFlg = "disabled";
var CuttingRecptDeleteFlg = "disabled";
var CuttingRecptPrintFlg = "disabled";
var ValidateProductionStore = "False";
var ValiCutBudApp = '';
var ValiCutBudAppSam = '';
var retchk = '';
...
$(document).ready(function () {
   ...
   // LoadCuttingRecptNoDDL("#ddlCutRcpNo");
    ...
    var fill = localStorage.getItem('CuttingReceiptMainFilter');
    ...
    $(document).on('keyup', '.txtrcdqty', function () {
        ...
        if (Balqty < qty) {
            ...
            var msg = 'Receipt quantity should not exceed Balance quantity...';
            var flg = 4;
            var mod = 1;
            var ur = "";
            AlartMessage(msg, flg, mod, ur);
    ...
    $('#tblMainGrid').on('click', 'tr', function (e) {
        ...
        var ProcessOrdNo = data.CuttingReceiptNo;
...
function LoadData(companyid, ordertype, interexter, fromdate, todate) {
    ...
    var menufilter = recptid + ',' + OrderType + ',' + InterExter + ',' + fromdate + ',' + todate + ',' + companyid + ',' + jobno + ',' + ordno + ',' + refno + ',' + empid + ',' + unitid ;
    localStorage.setItem('CuttingReceiptMainFilter', menufilter);

    $.ajax({
        type: "POST",
        url: '/CuttingOrderReceipt/GetMaindt/',
        ...
        data: JSON.stringify({
            ...
            orderno:ordno, refno:refno, employeeid:empid, unitid:unitid}),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function LoadDataFromBack(companyid, ordertype, interexter, fromdate, todate) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            orderno: fillobj[7], refno: fillobj[8], employeeid: fillobj[9], unitid: fillobj[10]
        }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
...
$(document).on('click', '.CuttRecptPrint', function () {
    ...
    window.open("../ReportInline/Production/CuttingReceiptReportInline/CuttingReceiptReportInline.aspx?CuttingReceiptId=" + CuttingReceiptId + "&Companyid=" + compid + "&type=" + 'C');
});
...
function Add() {
    ...
    if (sumwgt.toFixed(3) > parseFloat(balwgt.toFixed(3))) {
        //alert('Please Check Total Issue,Receipt,Return Weight.. / Bal Wgt- ' + balwgt.toFixed(3));
        var msg = 'Please Check Total Issue,Receipt,Return Weight.. / Bal Wgt- ' + balwgt.toFixed(3);
        var flg = 4;
        var mod = 1;
        var ur = "";
        AlartMessage(msg, flg, mod, ur);
    ...
    if (ValiCutBudApp == 'Y' && OrderType=='W') {
        $.each(cuttingDetlist, function (e) {
            if (cuttingDetlist[e].Recqty > 0) {
                if (cuttingDetlist[e].Rate > 0 && cuttingDetlist[e].Rate <= cuttingDetlist[e].Apprate) {
            ...
            $.ajax({
                ...
                success: function (result) {
                   ...
                   // $("#btnAdd").attr("disabled", false);
                    ...
                    var flg = 1;
                    var mod = 0;
                    var ur = "/CuttingOrderReceipt/CuttingOrderReceiptIndex";
                    AlartMessage(msg, flg, mod, ur);
...
function LoadCuttingOrderReceipt() {
    ...
    $.ajax({
        type: "POST",
        url: '/CuttingOrderReceipt/ListCuttingOrder',
        //data: JSON.stringify({}),
...
function LoadCuttingOrderReceipt_LastWORK() {
    debugger;
...
function BarcodeScan() {
    ...
    var txtBarcodeScan_CuttingOrdNo = $('#txtBarcodeValue').val();

    for (var i = 0; i < MainList_barcodeScanList.length; i++) {

        if (MainList_barcodeScanList[i].CuttingOrdNo == txtBarcodeScan_CuttingOrdNo) {
            return;
        }
    }
    ...
    jQuery.each(MainList_barcode, function (i, val) {
        ...
        if (val.CuttingOrdNo == txtBarcodeScan_CuttingOrdNo) {

            var det = {
                BuyOrdMasId: val.BuyOrdMasId,
                OrderNo: val.OrderNo,
                CuttingOrdId: val.CuttingOrdId,
                JobNo: val.JobNo,
                ProdPrgNo: val.ProdPrgNo,
                CuttingOrdNo: val.CuttingOrdNo,
            ...
            LoadCuttingOrderReceipt_Barcode(MainList_barcodeScanList);
            $('#txtBarcodeValue').val("");
...
function LoadCuttingOrderReceipt_Barcode(list) {
    debugger;
...
function Close() {
    window.location.href = "/CuttingOrderReceipt/CuttingOrderReceiptIndex";
}
...
function Totwgt() {
    ...
    $.each(cuttingDetlist, function (e) {
        if (cuttingDetlist[e].Recqty > 0) {
            ...
            ipissuqty = ipissuqty + parseFloat(cuttingDetlist[e].Recqty);
            totwgt = totwgt + parseFloat(cuttingDetlist[e].Weight);
        }

    });

    $('#txtRecd').val(ipissuqty);
    $('#txtWgt').val(totwgt);
}