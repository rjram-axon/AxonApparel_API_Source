...
$(document).ready(function () {
    ...
    //$('#ddlproType').val(1);
    load();
    var fill = localStorage.getItem('ShortageProgramMenuFilter');
    debugger;
    ...
    $(document).on('click', '.btnprodplanadd', function () {
        ...
        if (ischecked == true) {
            ...
            if (txt == '') {
                ...
                $('#txtprogdateshort').val(currentdate);
    ...
    $(document).on('click', '.btnprodplaneditadd', function () {
        ...
        var table = $('#tblprodprocess').DataTable();

        var row = $(this).closest('tr');

        var data = $('#tblprodprocess').dataTable().fnGetData(row);
    ...
    $(document).on('click', '.btnprodplanedit', function () {
        ...
        var app = currentval[0]['Approved'];
        if (app == 'Y') {
            $("#btnupd").attr("disabled", true);
            //alert('Program has been Approved..Cannot Update this program..');
            var msg = 'Program has been Approved..Cannot Update this program...';
            var flg = 4;
            var mod = 1;
            var url = "";
            AlartMessage(msg, flg, mod, url);
    ...
    $(document).on('click', '.btnprodplandelete', function () {
        ...
        if (app == 'Y') {
            ...
            //alert('Program has been Approved..Cannot delete this program..');
            var msg = 'Program has been Approved..Cannot delete this program...';
            var flg = 4;
            var mod = 1;
            var url = "";
            AlartMessage(msg, flg, mod, url);
...
function fnloadProductionPlanning(currentval) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            //GMaxChk = 0;
            //for (var u = 0; u < prodplanningList.length; u++) {
            //    GMaxChk = prodplanningList[u].MaxChk;
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (oldprdprgno != newprdprgno) {
                //alert('PrdPrgNo has been changed...');
            ...
            var obj = {
                ...
                companyunitid: CompanyUnitId,//5,//prodplanningedit[0].CompanyUnit,// CompanyUnitId,
                companyid: CompanyId,
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.Value == 1) {
                        AddUserEntryLog('Planning', 'Shortage Programing', 'ADD', $('#txtwrkordno').val());
                        ...
                        //    $('#tblprodprocess').DataTable().destroy();
                        //} else {
                        //    window.location.href = "/ShortageProgram/ShortageProgramIndex";
                        //}

                        //  $('#myModal3').modal('hide');
                        // $('#myModal3').hide();
                        var maxno = $('#txtproseq').val();
                        // getbyID(eid, maxno);
                        if (closeMode == 1) {
                            $('#myModal3').modal('hide');
                            getbyID(comeid, maxno);
                        }
                        if (closeMode == 3) {
                            window.location.href = "/ShortageProgram/ShortageProgramIndex";
                        }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (oldprdprgnocomp != newprdprgnocomp) {
                ...
                var msg = 'ProductionProgram Company has been changed...';
                var flg = 4;
                var mod = 1;
                var url = "";
                AlartMessage(msg, flg, mod, url);
...
function loadData(Gp) {
    ...
    var menufilter = FDate + ',' + TDate + ',' + cmpid + ',' + buyerid + ',' + ordNo + ',' + unitid + ',' + RecNo + ',' + Gp + ',' + type + ',' + DispatchClosed;
    localStorage.setItem('ShortageProgramMenuFilter', menufilter);
...
function loadDataFromBack(Gp) {
    ...
    $.ajax({
        type: "POST",
        url: '/ProcessProgram/List',
        data: JSON.stringify({ FromDate: fillobj[0], ToDate: fillobj[1], CompanyId: fillobj[2], buyerid: fillobj[3], orderno: fillobj[4], cmpnyunitid: fillobj[5], refno: fillobj[6], prgmtype: fillobj[7], Ordertype: fillobj[8], DispatchClosed: fillobj[9] }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
...
function getbyideditopen(id) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ Id: id, MaxId: GMaxChk }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
            ...
            var ctry = [];
            ctry = prodplanningedit;
            ctry = $.grep(ctry, function (e) {
                if (e.MaxChk == GMaxChk) {
                    return e;
                }
            });
            prodplanningedit = ctry;
            ...
            GMaxChk = prodplanningedit[0].MaxChk;
...
function getprodlist(id) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            var date = prodedit[0].prodpgmdate;
            ...
            $('#txtprogdateshort').val(moment(date).format('DD/MM/YYYY'));
            $('#remarksli').val(prodedit[0].Remarks);

            //var Dispatchchecked = prodplanningedit[0].Despatch_Closed;
            //if (Dispatchchecked) {
            //    DispatchClosed = "Y";
            //}
            //else {
...
function getproddeletelist(id) {
    $.ajax({
        ...
        success: function (json) {
            ...
            var date = proddelete[0].prodpgmdate;
...
function DeleteOpen(id) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ Id: id, Maxno: GMaxChk }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
...
function getbyiddeleteopen(id) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            var ctryd = [];
            ctryd = prodplanningedelete;
            ctryd = $.grep(ctryd, function (e) {
                if (e.MaxChk == GMaxChk) {
                    return e;
                }
            });
            prodplanningedelete = ctryd;
    ...
    if (tst == 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.Value == 1) {
                    AddUserEntryLog('Planning', 'Shortage Programing', 'UPDATE', $('#txtwrkordno').val());
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.Value == true) {
                        AddUserEntryLog('Planning', 'Shortage Programing', 'DELETE', $('#txtwrkordno').val());
...
function getprodAutoItemlist() {
    debugger;
    inputlist = [];
    outputlist = [];
    var jobno = $('#txtwrkordno').val();
    $.ajax({
        type: "POST",
        url: '/ProcessProgram/GetProdAutoItemList/',
        data: JSON.stringify({ Id: ProcessId, JobNo: jobno }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
            debugger;
