...
function LoadReport() {
    ...
    if (UType == "U") {
        ...
        SupplierId = $('#ddlBuyer').val();
    }

    var OType = $('input[name="MOType"]:checked').attr('value');    
   // var VType = $('input[name="VType"]:checked').attr('value');

    window.open("../Reports/Stores/StockOutwardReport.aspx?CompId=" + CmpID + "&BuyerID=" + BuyID + "&RefNo=" + RefNo + "&OrderNo=" + OrdNo + "&StyleID=" + StyleID + "&ItemId=" + ItemID + "&FromDate=" + FDate + "&ToDate=" + TDate + "&SupplierId=" + SupplierId + "&OType=" + OType + "&UType=" + UType);
    // + "&VType=" + VType
}
