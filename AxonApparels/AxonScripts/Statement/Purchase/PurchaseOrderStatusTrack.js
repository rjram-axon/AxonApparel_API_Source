...
$(document).ready(function () {
    ...
    LoadSupplierDDL("#ddlMSupplier,#ddlMDeli");
...
function LoadReport() {
    ...
    var RecNo = "";
    var RNo = $('select#ddlMRECNo option:selected').val();
    ...
    if (RNo == 0) {
        ...
        RecNo = $('select#ddlMRECNo option:selected').text();
    }
    ...
    if (Prc == 0) {
        ...
        Process = $('select#ddlMProcess option:selected').text();
    }


    var DeliID = $('#ddlMDeli').val();
    ...
    var ChkPOCancel = $('#ChkPOCancel').prop('checked');
    var Canceltype = 'Y'
    if (ChkPOCancel == false) {
        Canceltype = 'N'
    }

    var ChkGrn = $('#ChkGrn').prop('checked');
    var Grntype = 'Y'
    if (ChkGrn == false) {
        Grntype = 'N'
    }

    var ChkGrnRet = $('#ChkGrnRet').prop('checked');
    var GrnRettype = 'Y'
    if (ChkGrnRet == false) {
        GrnRettype = 'N'
    }

    window.open("../Reports/Stores/PurchaseOrderStatusTrack.aspx?CompanyID=" + CmpID + "&OrderNo=" + OrdNo + "&RefNo=" + RefNo + "&StyID=" + StyID + "&SuppID=" + SuppID + "&PoNo=" + PoNo + "&FromDate=" + FDate + "&ToDate=" + TDate + "&OType=" + OType + "&POType=" + POType + "&LType=" + LType + "&Recno=" + RecNo + "&Deli=" + DeliID + "&itemid=" + ItemID + "&colorid=" + Colorid + "&sizeid=" + Sizeid + "&process=" + Process + "&Canceltype=" + Canceltype + "&Grntype=" + Grntype + "&GrnRettype=" + GrnRettype);
    //window.open("../Reports/Stores/PurchaseOrderTracking.aspx");
}
function getDate() {
...
function ListPoNo() {
    ...
    $.ajax({
        url: "/PurchaseOrderMain/GetPoNoTrack",
        data: JSON.stringify(),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function ListRecNo() {
    ...
    $.ajax({
        url: "/PurchaseOrderMain/GetRecNoTrack",
        data: JSON.stringify(),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        ...
        success: function (result) {
            ...
            if (result.Status == 'SUCCESS') {
                ...
                //Po No
                $(ddlMRECNo).append($('<option/>').val('0').text('--Select Receiptno--'));
                $.each(po, function () {
                    $(ddlMRECNo).append($('<option></option>').val(this.pur_ord_id).text(this.pur_ord_no));
                });
...
function myPONo(Val) {
    ...
    var foo = [];
    MPONo = 0;
    $('#ddlMPONo :selected').each(function (i, selected) {
        foo[i] = $(selected).val();
