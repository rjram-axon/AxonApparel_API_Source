...
function LoadReport() {
    ...
    window.open("../Reports/Stores/PoVsGrnReport.aspx?CompanyID=" + CmpID + "&BuyID=" + BuyID + "&SuppID=" + SuppID +
        "&PoNo=" + PoNo + "&FromDate=" + FDate + "&ToDate=" + TDate + "&OType=" + OType + "&POType=" + POType + "&LType=" + LType);
}
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Status == 'SUCCESS') {
                ...
                //Po No
                $(ddlMPONo).append($('<option/>').val('0').text('--Select Receiptno--'));
                $.each(po, function () {
