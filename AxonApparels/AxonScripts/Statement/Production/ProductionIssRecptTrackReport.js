...
function LoadReport() {
    ...
    var IssuNo = $('#ddlIssuNo').val();
    if (IssuNo == '0') {
       var  Issu = '';
    } else {
        var Issu = $('#ddlIssuNo option:selected').text();
    }
    ...
    window.open("../Reports/Production/ProductionIssRecTracking/ProductionIssRptTrackingStatement.aspx?fdate=" + FDate + "&tdate=" + TDate + "&Compid=" +
        CmpID + "&processid=" + procid + "&suppid=" + processorid + "&Masid=" + OrdNo + "&Ref=" + RefNo + "&unitid=" + UnitID + "&Buyid=" + BuyID + "&Ordtype=" + protype + "&IssuNo=" + Issu);
}
...
function LoadIssuNoddl() {
    ...
    $.ajax({
        url: "/CuttingOrder/GetAllIssueNo/",
        data: JSON.stringify({}),
        type: "POST",
        ...
        success: function (result) {
            ...
            $.each(obj, function (i, el) {
                ...
                if (!username[el.CuttingOrdId]) {
                    username[el.CuttingOrdId] = true;
                    usernameary.push(el);
                }
            });

            $('#ddlIssuNo').empty();
            $('#ddlIssuNo').append($('<option/>').val('0').text('--Select IssueNo--'));
            $.each(usernameary, function () {
                $('#ddlIssuNo').append($('<option></option>').val(this.CuttingOrdId).text(this.CuttingOrdNo));
            });
